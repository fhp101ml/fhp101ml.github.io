...
![Grafo Arquitectura KDD](kdd_graph.png)
...
- **Generic/Specialist Agents (Fases > 1):** A medida que la fase avanza a perfilado, EDA o modelado, el router despertará a agentes que tienen capacidades de escribir Python (usando LLMs a los que se asocia el Tool_Node). Estos agentes delegarán la ejecución del código al [Sandbox Docker], recuperarán las respuestas y se las explicarán al usuario. En la v0.6.0 el sistema bifurca esta capacidad entre el **Experto Científico de Datos** (EDA) y el **Ingeniero de Datos MLOps** (Data Preparation).
...
- **v0.6.0** (Actual): Transición automatizada a Fase 4 (Data Preparation). Incorporación de un Agente Especialista en limpieza e imputación de variables (MLOps) con re-enrutamiento condicional asíncrono desde el EDA. Arquitectura multi-agente técnica estabilizada.
- **v0.5.0**: Fase 6 (EDA Dinámico y Sandbox Efímero) y completado final de la Fase 5 (Recarga de historial desde bases de datos vectoriales persistentes). Agente de código Python plenamente funcional, enrutamiento condicional perfecto en el Session Manager.
